...
bg = pygame.image.load(r"Backgrounds_1.png").convert()
bgWidth, bgHeight = bg.get_rect().size

score=0
health=10
x=input("Enter Name:")
cursor.execute("Insert into LEVELONE values(""\'"+ x +"\',""\'"+ str(score) +"\',""\'"+ str(health) +"\','no')")
mydb.commit()
...
class mask(object):
    mask = pygame.image.load(r'mask.png')
    ...
    def draw(self,screen): 
          if self.visible==True:
                DS.blit(self.mask,(self.x,self.y))
...
man=player(32,530,80,80)
m=mask(800,530,64,64)
enemies=[]
maxenemies=3
for i in range(maxenemies):
      enemies.append(enemy(random.randint(100,500), random.randint(480,530), 64, 64,936))
manRadius = 32
manPosX = manRadius
playerPosX = manPosX
shootloop=0
...
while True:
      ...
      if m.visible==True:
              if man.hitbox[1] < m.hitbox[1]+ m.hitbox[3] and man.hitbox[1] + man.hitbox[3] > m.hitbox[1]:
                      if man.hitbox[0] + man.hitbox[2]> m.hitbox[0] and man.hitbox[0]< m.hitbox[0] + m.hitbox[2]:
                          m.hit()
                          score+=5
                          Collectible='Yes'
      ...
      pygame.display.update()
      if Collectible=='Yes':
            cursor.execute("Update LEVELONE set Score=""\'"+ str(score) +"\', Health=""\'"+ str(health) +"\', Collectible='Yes' where Name=""\'"+ x +"\'")
            mydb.commit()
            pygame.quit()
            sys.exit()
      if man.health==0:
            cursor.execute("Update LEVELONE set Score=""\'"+ str(score) +"\', Health=""\'"+ str(health) +"\', Collectible='No' where Name=""\'"+ x +"\'")
            mydb.commit()
            pygame.quit()
            sys.exit()
      if Collectible=="No":
