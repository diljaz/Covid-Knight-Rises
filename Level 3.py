...
bg = pygame.image.load(r"Backgrounds_3.png").convert()
bgWidth, bgHeight = bg.get_rect().size

score=0
health=10
x=input("Enter Name:")
cursor.execute("Insert into LEVELTHREE values(""\'"+ x +"\',""\'"+ str(score) +"\',""\'"+ str(health) +"\','no')")
mydb.commit()
...
class glove(object):
    glove = pygame.image.load(r'gloves.png')
    ...
    def draw(self,screen): 
          if self.visible==True:
                DS.blit(self.glove,(self.x,self.y))
...
man=player(32,605,80,80)
g=glove(random.randint(800,936),605,64,64)
enemies=[]
maxenemies=7
for i in range(maxenemies):
      enemies.append(enemy(random.randint(100,500), random.randint(550,605), 64, 64,936))
manRadius = 32
manPosX = manRadius
playerPosX = manPosX
shootloop=0
...
while True:
      ...
      if g.visible==True:
              if man.hitbox[1] < g.hitbox[1]+ g.hitbox[3] and man.hitbox[1] + man.hitbox[3] > g.hitbox[1]:
                      if man.hitbox[0] + man.hitbox[2]> g.hitbox[0] and man.hitbox[0]< g.hitbox[0] + g.hitbox[2]:
                          g.hit()
                          score+=5
                          Collectible='Yes'
      ...
      pygame.display.update()
      if Collectible=='Yes':
            cursor.execute("Update LEVELTHREE set Score=""\'"+ str(score) +"\', Health=""\'"+ str(health) +"\', Collectible='Yes' where Name=""\'"+ x +"\'")
            mydb.commit()
            pygame.quit()
            sys.exit()
      if man.health==0:
            cursor.execute("Update LEVELTHREE set Score=""\'"+ str(score) +"\', Health=""\'"+ str(health) +"\', Collectible='No' where Name=""\'"+ x +"\'")
            mydb.commit()
            pygame.quit()
            sys.exit()
      if Collectible=="No":
