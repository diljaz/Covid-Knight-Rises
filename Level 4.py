...
bg = pygame.image.load(r"Backgrounds_4.png").convert()
bgWidth, bgHeight = bg.get_rect().size

score=0
health=10
x=input("Enter Name:")
cursor.execute("Insert into LEVELFOUR values(""\'"+ x +"\',""\'"+ str(score) +"\',""\'"+ str(health) +"\','no')")
mydb.commit()
...
class kit(object):
    kit = pygame.image.load(r'kit.png')
    ...
    def draw(self,screen): 
          if self.visible==True:
                DS.blit(self.kit,(self.x,self.y))
...
man=player(32,550,80,80)
k=kit(random.randint(1100,1216),550,64,64)
enemies=[]
maxenemies=9
for i in range(maxenemies):
      enemies.append(enemy(random.randint(100,500), random.randint(450,550), 64, 64,1216))
manRadius = 32
manPosX = manRadius
playerPosX = manPosX
shootloop=0
...
while True:
      ...
      if k.visible==True:
              if man.hitbox[1] < k.hitbox[1]+ k.hitbox[3] and man.hitbox[1] + man.hitbox[3] > k.hitbox[1]:
                      if man.hitbox[0] + man.hitbox[2]> k.hitbox[0] and man.hitbox[0]< k.hitbox[0] + k.hitbox[2]:
                          k.hit()
                          score+=10
                          Collectible='Yes'
      ...
      pygame.display.update()
      if Collectible=='Yes':
            cursor.execute("Update LEVELFOUR set Score=""\'"+ str(score) +"\', Health=""\'"+ str(health) +"\', Collectible='Yes' where Name=""\'"+ x +"\'")
            mydb.commit()
            pygame.quit()
            sys.exit()
      if man.health==0:
            cursor.execute("Update LEVELFOUR set Score=""\'"+ str(score) +"\', Health=""\'"+ str(health) +"\', Collectible='No' where Name=""\'"+ x +"\'")
            mydb.commit()
            pygame.quit()
            sys.exit()
      if Collectible=="No":
